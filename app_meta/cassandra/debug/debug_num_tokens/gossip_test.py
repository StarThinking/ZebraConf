...
class TestGossip(Tester):
    ...
    def _start_nodes_in_parallel(self, nodelist):
        ...
        for node in nodelist:
            if not node.is_running():
                logger.warn("[msx] node {} is not running".format(node.name))
                sys.exit(1)
                assert node.is_running()
            else:
                logger.warn("[msx] node {} is running".format(node.name))
    ...
    def _create_2dc_cluster(self):
        ...
        logger.warn("[msx] node1-4 started")
        
        # Stop cluster
        for node in cluster.nodelist():
            node.stop()
        
        logger.warn("[msx] node1-4 stopped")
    ...
    def test_2dc_parallel_startup(self):
        ...
        logger.warn("[msx] node1 node3 started")

        # Start non-seeds in parallel
        self._start_nodes_in_parallel([node2, node4])
        logger.warn("[msx] node2 node4 started")
    ...
    def test_2dc_parallel_startup_one_seed(self):
        ...
        # Start one seed (of two)_start_nodes_in_parallel
