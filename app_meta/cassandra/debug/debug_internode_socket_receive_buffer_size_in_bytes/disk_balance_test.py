...
class TestDiskBalance(Tester):
    ...
    def _test_disk_balance_replace(self, same_address):
        ...
        node2.set_configuration_options(values={'internode_socket_receive_buffer_size_in_bytes': 4096})
        node4.set_configuration_options(values={'internode_socket_receive_buffer_size_in_bytes': 4096})
        cluster.start()
        ...
        logger.warn("[msx] -----------------------1------------------------")
        logger.warn("[msx] self.assert_balanced for node1")
        self.assert_balanced(node1)
        
        logger.warn("[msx] self.assert_balanced for node2")
        self.assert_balanced(node2)
        
        logger.warn("[msx] self.assert_balanced for node3")
        self.assert_balanced(node3)
        
        logger.warn("[msx] self.assert_balanced for node4")
        self.assert_balanced(node4)
        
        logger.warn("[msx] Stopping and removing node2")
        ...
        logger.warn("[msx] before node5 starting")
        ...
        logger.warn("[msx] Checking replacement node is balanced");
        logger.warn("[msx] -----------------------3------------------------")
    ...
    def assert_balanced(self, node):
        ...
        logger.warn("[msx] assert_balanced begin, new_sums = {}, old_sums = {}".format(new_sums, old_sums))
        ...
        for _ in range(20):
            logger.warn("[msx] repeat");
            old_sums = new_sums
            new_sums = []
            for sstabledir in node.get_sstables_per_data_directory('keyspace1', 'standard1'):
                logger.warn("[msx] sstabledir = {}".format(sstabledir))
                sum = 0
                for sstable in sstabledir:
                    logger.warn("[msx] sstable = {}".format(sstable))
                    sum = sum + os.path.getsize(sstable)
                new_sums.append(sum)
                logger.warn("[msx] assert_balanced middle, new_sums = {}, old_sums = {}".format(new_sums, old_sums))
            if new_sums == old_sums:
                break
            sleep(2)

        assert len(new_sums)
        logger.warn("[msx] new_sums = {}, old_sums = {}".format(new_sums, old_sums))
