...
public class ConfAgent {
    ...
    public static class ConfAgentHashMap<K,V> extends HashMap<K,V> {
    }
    
    public static class ConfAgentHashSet<E> extends HashSet<E> {
    }
	...
	public Set<Configuration> confSet = new ConfAgentHashSet<Configuration>();
        public int index = 0;
    ...
    private static Set<Configuration> unidentifiedConfSet = new ConfAgentHashSet<Configuration>();
    private static Set<Configuration> externalConfSet = new ConfAgentHashSet<Configuration>();
    private static Map<Configuration, Map<String, Long>> ocsTimestampMap = new ConfAgentHashMap<Configuration, Map<String, Long>>();
    ...
    private static void refreshExternalConf() {
        while (true) {
            Set<Configuration> newAddedExternalConfs = new ConfAgentHashSet<Configuration>();
    ...
    public static void updateExternalConfWhenCopy(Configuration o_c) {
        ...
        if (entryList.size() > 0) {
            // using config specific functions: 
            /* hadoop conf */
            ...
            /* flink conf */
            for (String key : o_c.confData.keySet()) {
                // directly call ConfAgent.whichV() to update 
                o_c.confData.put(key, ConfAgent.whichV(o_c, key, o_c.confData.get(key)));
            }
        }
    }
