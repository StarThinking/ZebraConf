# process single hypothesis tar balls
...
# check if conjunction flag is enabled on all containers
for i in $(seq 0 19); do ssh node-$i 'for my_cont in $(docker ps -aq); do docker exec $my_cont bash -c "cat /root/ZebraConf/runner/conjunction_enable"; done' ; done | sort | uniq -c
