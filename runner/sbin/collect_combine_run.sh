...
do 
    if [ $compress -eq 1 ]; then
        ...
        ssh node-$i '/root/vm_images/ZebraConf/container_sbin/local/cp_from_cont_2_local_by_key.sh combine_run /root/ZebraConf/runner/log/ /root/vm_images/ZebraConf/runner/log/' &
    fi
    pids+=($!)
done
